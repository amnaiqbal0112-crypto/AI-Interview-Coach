...
def ask_claude(system: str, user_msg: str, max_tokens: int = 1500) -> str:
    """Uses Groq (llama-3.3-70b) — called fresh every time, no cache."""
...
def page_dashboard():
    ...
    with b2:
        ...
        # Use real data or show sample if no interviews yet
        if domain_avg:
            chart_labels = list(domain_avg.keys())
            chart_values = list(domain_avg.values())
        else:
            chart_labels = ["Programming", "Data Structures", "Machine Learning", "System Design", "Communication"]
            chart_values = [85, 78, 82, 45, 70]
        ...
        if not domain_avg:
            st.caption("📌 Sample data — complete interviews to see your real stats")
...
def page_settings():
    ...
    st.info("🤖 Powered by Groq AI (llama-3.3-70b) · Built with Streamlit")
